...
void EXEC_DEC_OFF(parser_t *parser, uint idx, uint8 type)
{
    uint8 *op = malloc(0xA);
    uint8 k = 0x0;
...
void EXEC_DEC_REG(parser_t *parser, uint8 type)
{
    uint8 *op = malloc(0x2);
    uint8 k = 0x0;
    ...
    if (type % 10 == REGISTER_8b) {
        op[k] = DEC_REGISTER8;
        op[k + 1] = BASE_DEC_REGISTER8 + (type / 10);
        ...
        op[k] = DEC_REGISTERH + (type / 10);
        k++;
        WriteOpcode(parser, SECTION_TEXT_V, op, k);
    }
}

void EXEC_DEC_OFFREG(parser_t *parser, uint idx, uint8 type)
{
    uint8 *op = malloc(0x3);
    uint8 k = 0x0;

    if (type % 10 != REGISTER_32b)
        pexit("Invalid register size on Dec instruction\n");
    parser->inp[idx][0x1] = RemoveBrackets(parser->inp[idx][0x1]); // we already know the instr type
    if (parser->control->OffSize[0x0] == BYTE_VALUE) {
        op[k] = DEC_OFFSET8;
        op[k + 1] = type / 10;
...
void EXEC_DEC(parser_t *parser, uint idx)
{
    if (len_tab(parser->inp[idx]) > 0x3)
        pexit("Invalid argument number for Dec, need 1 operand\n");
...
(type < TYPE_OP_IS_REG_MAX && !parser->control->offreg[0x0] && parser->control->OffSize[0x0] != UNDEFINED_OFF_VAL))
    ...
    if (type == TYPE_OP_IS_IMM || type == TYPE_OP_IS_IMM_FROM_SYM)
        pexit("Can't decrement an Immediate value\n");

    if (type == TYPE_OP_IS_OFF || type == TYPE_OP_IS_OFF_FROM_SYM)
        EXEC_DEC_OFF(parser, idx, type);
    if (type < TYPE_OP_IS_REG_MAX && parser->control->offreg[0x0])
        EXEC_DEC_OFFREG(parser, idx, type);
    if (type < TYPE_OP_IS_REG_MAX && !parser->control->offreg[0x0])
        EXEC_DEC_REG(parser, type);
}
